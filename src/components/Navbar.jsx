...
const Navbar = ({ searchTerm, setSearchTerm }) => {
  ...
  // Buttons ki styling (Mobile par fit aane ke liye thori chhoti ki hai)
  const actionBtnClass = "flex items-center gap-1 md:gap-2 px-2 md:px-4 py-2 rounded-xl transition-all duration-200 border border-gray-100 shadow-sm hover:shadow-md font-bold text-[10px] md:text-xs uppercase tracking-tight";

  return (
    <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
      <div className="w-full px-3 md:px-8 py-3 flex justify-between items-center gap-2 md:gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          ...
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            <div className="bg-[#1e293b] text-white w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-lg font-black text-sm md:text-lg shadow-lg group-hover:bg-[#b89146] transition-all">
              A
            </div>
        ...
        {/* Search Bar - Desktop Only (Hidden on mobile to show below) */}
        ...
        <div className="flex items-center gap-1 md:gap-3">
          ...
          {/* ✅ AAPKA ORIGINAL USER CHECK (Orders sirf login par dikhega) */}
          {user && (
          ...
          {/* Cart */}
          <Link to="/cart" className={`${actionBtnClass} bg-[#1e293b] text-white border-transparent`}>
      ...
      {/* ✅ MOBILE SEARCH BAR - Logo ke niche prominent nazar aayegi */}
